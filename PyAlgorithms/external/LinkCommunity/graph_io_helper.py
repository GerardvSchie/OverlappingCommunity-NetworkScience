...
def read_edge_list_unweighted(filename, node_type=str):
    ...
    with open(filename) as ifs:
        edge_list = list(map(lambda ele: ele.strip().split(), ifs.readlines()))
        edge_list = filter(lambda edge: edge[0] != edge[1], map(lambda edge: list(map(node_type, edge)), edge_list))
        print(edge_list)
...
def write_edge2cid(e2c, dirname, delimiter="\t"):
