	...
	if(dim==0) {
		...
		for(int i=0; i<dim; i++) {
			...
			for(int j=0; j<vertices[i]->links->size(); j++) {
				...
				//hp+=vertices[i]->links->w[j].second;
			}
			
			/*
			hp/=vertices[i]->stub_number;
			neigh_av_w.push_back(hp);*/
		...
		for(int i=0; i<dim; i++) {
			...
			for(int j=0; j<vertices[i]->links->size(); j++) {
				...
				/*{	
					...
					double  Wij=vertices[i]->links->w[j].second;
					double  Aij=vertices[i]->links->w[j].first;
					
					
					double xi = Wij * Aij/ vertices[i]->strength;
					double xj = Wij * Aij/ vertices[j]->strength;
					
					
					double g1= pow(1 -xi, vertices[i]->stub_number-1);
					double g2 = pow(1 - xj, vertices[i]->stub_number-1);
					
					double ll= log_together(-log(g1)-log(g2), 2);
					
					
					cout<<"node: "<<id_of(i)<<" "<<vertices[i]->strength<<" "<<xi<<" "<<Wij<<endl;
					
					cout<<xi<<" "<<xj<<" ll "<<ll<<" "<<g1<<" "<<g2<<endl;
					
					
					vertices[i]->links->w[j].second= -log(ll);
