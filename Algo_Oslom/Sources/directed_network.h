...
void static_network::set_proper_weights() {
	...
	if(dim==0) {
		...
		for(int i=0; i<dim; i++) {
			...
			if(vertices[i]->stub_number!=0) {
				
				sen_nodes++;
				eta+=vertices[i]->strength / vertices[i]->stub_number;
				//cout<<vertices[i]->strength<<" "<<id_of(i)<<" "<<i<<endl;
			
			}
		}
		
		eta/=sen_nodes;*/
		//cout<<"eta: "<<eta<<" computed over "<<sen_nodes<<endl;
		...
		for(int i=0; i<dim; i++) {
			...
			vertices[i]->in_original_weights.clear();
			
			for(int j=0; j<vertices[i]->links->size(); j++) {
				vertices[i]->in_original_weights.push_back(vertices[i]->inlinks->w[j].second);
			}
		...
		*/
		...
		for(int i=0; i<dim; i++) {
			...
			for(int j=0; j<vertices[i]->outlinks->size(); j++) {
				...
				/*
				cout<<id_of(i)<<"<-id: strenght and stuff "<<id_of(neigh)<<endl;
				cout<<w1<<" "<<w2<<" "<< 2./(1./w1 + 1./w2)<<" wiegts"<<endl;
				cout<<vertices[i]-> instrength<<" "<<vertices[i]-> instub_number<<" "<<vertices[i]-> outstrength<<" "<<vertices[i]-> outstub_number<<" instrength, instub outstre, outstubs"<<endl;
				cout<<vertices[neigh]-> instrength<<" "<<vertices[neigh]-> instub_number<<" "<<vertices[neigh]-> outstrength<<" "<<vertices[neigh]-> outstub_number<<endl;
				*/
				
				/*double W= vertices[i]->links->w[j].second;
				
				double a1= -(vertices[i]->stub_number -1) * log(1. - W/vertices[i]->strength);
				
				if(vertices[i]->stub_number==1)
					a1=0.5;
				
				double a2= -(vertices[vertices[i]->links->l[j]]->stub_number -1) * log(1. - W/vertices[vertices[i]->links->l[j]]->strength);
				
				if(vertices[vertices[i]->links->l[j]]->stub_number==1)
					a2=0.5;
				
				//ratios.push_back(W/vertices[i]->strength);
				//*/
				//vertices[i]->links->w[j].second= -log( log_together(a1+a2, 2));
				//vertices[i]->links->w[j].second= max(a1,a2);
				
				
				//vertices[i]->links->w[j].second= -log( log_together(vertices[i]->links->w[j].second*(1./w1 + 1./w2), 2));
				//vertices[i]->links->w[j].second/=  0.5*(vertices[i]->strength / vertices[i]->stub_number + vertices[vertices[i]->links->l[j]]->strength / vertices[vertices[i]->links->l[j]]->stub_number);
				//int Nnn=vertices[i]->links->l[j];
				//double mah=sqrt(neigh_av_w[i] * neigh_av_w[Nnn]);
				//cout<<w1<<" "<<w2<<" "<< 2./(1./w1 + 1./w2)<<" "<<vertices[i]->strength<<" "<<vertices[i]->stub_number<<" "<<vertices[vertices[i]->links->l[j]]->strength<<" "
				//<<vertices[vertices[i]->links->l[j]]->stub_number<<" "<<vertices[i]->links->w[j].second<<" "<<vertices[i]->original_weights[j]<<" -> "<<vertices[i]->links->w[j].first<<endl;
				//vertices[i]->links->w[j].second/=  w1 * w2 /mah ;
				//vertices[i]->links->w[j].second= -log( log_together(vertices[i]->links->w[j].second*(1./w1 + 1./w2), 2));
				//vertices[i]->links->w[j].second/= vertices[i]->strength / vertices[i]->stub_number * vertices[vertices[i]->links->l[j]]->strength / vertices[vertices[i]->links->l[j]]->stub_number / eta;
				
				//vertices[i]->links->w[j].second= -log(ran4());
			}
			//cherr();
		}
		...
		//ofstream pp("ratios");
		//print_cumulative(ratios, "ratios", 100);
		//draw("here");
		//cherr();
		//************************************************************
...
int static_network::propagate_distances(deque<int> & new_shell, set<int> & already_gone, deque<pair<int, int> > & distances_node, int shell, deque<double> & ML, int & reached, int step) {
	...
	for(int i=0; i<new_shell.size(); i++) for(int j=0; j<vertices[new_shell[i]]->outlinks->size(); j++) {
...
int static_network::draw_consecutive() {
	...
	* /
	...
	if (true) {
		...
		for (int i=0; i<vertices.size(); i++)
			for (int j=0; j<vertices[i]->outlinks->size(); j++) if(vertices[i]->id_num <= vertices[vertices[i]->outlinks->l[j]]->id_num)
				cout<<i<<"\t"<<vertices[i]->outlinks->l[j]<<"\t"<<vertices[i]->outlinks->w[j].first<<"\t"<<vertices[i]->outlinks->w[j].second<<endl;
